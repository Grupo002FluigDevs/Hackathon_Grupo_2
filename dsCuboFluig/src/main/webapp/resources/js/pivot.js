...
(function() {
  ...
  locales = {
    ...
    br: {
        ...
        renderers: renderers,
        localeStrings: {
          renderError: "Ops, Ocorreu um erro ao redenrizar a tabela.",
          computeError: "Ops, Ocorreu um erro ao processar as informações da tabela",
          uiRenderError: "Ops, Ocorreu um erro ao redenrizar a tabela.",
...
//    result.className = "pvtTable";
    result.className = "pvtTable table table-bordered";
    result.id = "pvInfo";
    ...
    for (j in colAttrs) {
      ...
      th = document.createElement("th");
//      th.className = "pvtAxisLabel";  alterado
      th.textContent = c;
      tr.appendChild(th);
      for (i in colKeys) {
...
//        th.className = "pvtAxisLabel"; alterado
  ...
  $.fn.pivot = function(input, opts) {
    ...
    defaults = {
      ...
      localeStrings: locales.br.localeStrings
    };
    opts = $.extend(defaults, opts);
    result = null;
    try {
  ...
  $.fn.pivotUI = function(input, inputOpts, overwrite, locale) {
    ...
    try {
      ...
      uiTable = $("<table cellpadding='5' class='table table-bordered'>");
      rendererControl = $("<td>");
      renderer = $("<select class='pvtRenderer form-control input-sm'>").appendTo(rendererControl).bind("change", function() {
        return refresh();
      });
      ...
      _fn = function(c) {
        ...
        if (keys.length > opts.menuLimit) {
          ...
          btns.append($("<button class='btn btn-info'>").html(opts.localeStrings.selectAll).bind("click", function() {
            return valueList.find("input:visible").prop("checked", true);
          }));
          btns.append($("<button class='btn btn-info'>").html(opts.localeStrings.selectNone).bind("click", function() {
            return valueList.find("input:visible").prop("checked", false);
          }));
          btns.append($("<input>").addClass("pvtSearch form-control input-sm").attr("placeholder", opts.localeStrings.filterResults).bind("keyup", function() {
            var filter;
        ...
        $("<p>").appendTo(valueList).append($("<button class='btn btn-info'>").text("OK").bind("click", updateFilter));
      ...
      aggregator = $("<select class='pvtAggregator form-control input-sm'>").bind("change", function() {
        return refresh();
      });
        ...
        return function() {
          ...
          if (numInputsToProcess !== 0) {
            ...
            for (x = _m = 0; 0 <= numInputsToProcess ? _m < numInputsToProcess : _m > numInputsToProcess; x = 0 <= numInputsToProcess ? ++_m : --_m) {
              newDropdown = $("<select class='pvtAttrDropdown form-control input-sm'>").append($("<option>")).bind("change", function() {
                return refresh();
              });
